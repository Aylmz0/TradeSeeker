...
def main():
    ...
    # Group by Momentum
    mom_groups = []
    for name, group in df_matched.groupby("momentum"):
        stats = get_stats(group)
        stats["Momentum"] = name
        mom_groups.append(stats)
    df_mom = pd.DataFrame(mom_groups)
    ...
    with open(output_report, "w") as f:
        ...
        # 1. Regimes
        choppy_data = df_matched[df_matched["regime"] == "CHOPPY"]
        if not choppy_data.empty and choppy_data["pnl"].sum() < 0:
            insights.append(
        ...
        # 2. Alignment
        ct_data = df_matched[df_matched["trend_alignment"] == "counter_trend"]
        if not ct_data.empty and ct_data["pnl"].sum() < 0:
            ct_pnl = ct_data["pnl"].sum()
            insights.append(
                f"- **[CRITICAL] Counter-Trend Disadvantage**: Counter-trend entries generated a total PnL of **${ct_pnl:.2f}** with a win rate of **{(ct_data['pnl'] > 0).mean()*100:.1f}%**. Counter-trend trades are highly toxic compared to trend-following setups. We should increase MIN_CONFIDENCE for counter-trend entries to **0.75**."
            )

        # 3. Volume
        poor_vol_data = df_matched[df_matched["vol_cat"].isin(["POOR (0.7x-1.2x)", "WEAK (<0.7x)"])]
        if not poor_vol_data.empty:
        ...
        # 4. Exit reason
        ai_close_data = df_matched[df_matched["close_category"] == "AI Close Signal"]
        if not ai_close_data.empty:
