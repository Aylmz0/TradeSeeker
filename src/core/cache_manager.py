...
def fetch_all_indicators_parallel(
    ...
    def fetch_indicators_for_coin(coin: str) -> tuple:
        ...
        try:
            ...
            return (
                coin,
                {"3m": indicators_3m, "15m": indicators_15m, htf_interval: indicators_htf},
                sentiment,
            )
        except Exception as e:
    ...
    with ThreadPoolExecutor(max_workers=10) as executor:
        futures = {
            executor.submit(fetch_indicators_for_coin, coin): coin for coin in available_coins
        }
...
def fetch_all_indicators_with_cache(
    ...
    with ThreadPoolExecutor(max_workers=10) as executor:
        futures = {
            executor.submit(fetch_indicators_for_coin_cached, coin): coin
