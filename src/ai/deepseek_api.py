    ...
    def _build_system_prompt(self) -> str:
        ...
        system_structure = {
            ...
            "analysis_process": [
                ...
                "4b. CRITICAL: If ML BUY or SELL exceeds 50%, this is a HIGH-CONFIDENCE signal. Weight ML as Tier-1 indicator. Only override if technicals show clear contradiction (e.g., ML BUY >50% but 15m structure is LH_LL in bearish 1h).",
            ...
            "state_vector_schema": {
                "ml_consensus": {
                    ...
                    "usage": "Use ML as you would any other indicator (RSI, MACD, Volume). Consider it alongside your technical analysis. When ML and technicals agree, confidence increases. When they disagree, weigh the evidence. CRITICAL THRESHOLD: ML >=50% is a STRONG SIGNAL - Model has HIGH conviction (>60% probability). When BUY/SELL exceeds 50%, treat this as Tier-1 confirmation. If technicals also align, proceed with higher confidence. Interpretation: BUY/SELL > 50% = STRONG SIGNAL - high confidence entry opportunity. BUY/SELL > 40% = Model favors that direction but conviction is MODERATE. HOLD < 40% = Model has directional conviction (BUY+SELL > 60%); check which side dominates. HOLD > 50% = Model is uncertain; ignore ML, rely on technicals only.",
                },
            ...
            "few_shot_examples": [
                {
                    ...
                    "output_example": {
                        "CHAIN_OF_THOUGHTS": "Systematic execution: 1h Trend + 15m Momentum/Structure + 3m Timing + ML Consensus. Logic precedes decision.\\n\\nXRP: 1h Bullish (Price $0.54 > EMA20 $0.52, RSI 62, ADX 32 MODERATE). 15m Bullish (Structure HH_HL, Momentum STRENGTHENING, BB_Width 5.2% expanding). 3m Bullish (RSI 64, Volume 1.8x). ML Consensus: BUY 38%. Logic: Full technical alignment across all timeframes with expanding volume. ML supports direction. Decision: BUY_TO_ENTER.\\n\\nSOL (OPEN LONG): 1h Bearish (ADX 35 STRONG, Bullish Trend Broken). 15m Bearish (Structure LH_LL, Price < EMA20). Position Risk: Erosion SIGNIFICANT (55% profit decay). Reversal Score: STRONG (7/10). ML Consensus: SELL 42%. Logic: Technical invalidation confirmed by 15m structure reversal. Profit erosion above 50%. Decision: CLOSE_POSITION.\\n\\nTRX: 1h Bullish (Price $0.12 > EMA20 $0.118). 15m Bearish (Extreme dip, RSI 22 OVERSOLD, Price at BB Lower Band). 3m Reversing (MACD Bullish Cross). ML Consensus: BUY 35%. Counter-trade risk: MEDIUM (oversold RSI + BB lower band + 3m reversal). Logic: Buying the dip in 1h bull regime with structural support. Decision: BUY_TO_ENTER (Counter-Trend Dip).\\n\\nDOGE: 1h Bullish but Price Location UPPER_10 (Resistance). 15m Momentum WEAKENING. ML Consensus: HOLD 38%, BUY+SELL = 62% (directional conviction split). Logic: Buying at resistance with weakening momentum. Wait for pullback or breakout confirmation. Decision: HOLD.\\n\\nASTER: Volume Ratio 0.12x (< 0.30x Hard Block). BB Squeeze (Volatility 1.2%). Decision: HOLD (Volume blocks entry).",
                        "DECISIONS": {
                            "XRP": {
                                "signal": "buy_to_enter",
