...
class EnhancedContextProvider:
    ...
    def get_performance_insights(
        ...
        # Calculate profitability index by coin
        coin_performance = {}
        ...
        profitable_coins = [
            coin for coin, stats in coin_performance.items() if stats["total_pnl"] > 0
        ]
        if profitable_coins:
    ...
    def get_directional_feedback(self, trade_history: list, lookback: int = 20) -> dict[str, Any]:
        ...
        recent_trades = (
            trade_history[-lookback:] if len(trade_history) > lookback else trade_history
        )
        ...
        for direction, direction_stats in feedback.items():
            ...
            if trades > 0:
                ...
                direction_trades = [
                    t for t in recent_trades if t.get("direction", "").lower() == direction
                ]
                direction_profit = sum(
