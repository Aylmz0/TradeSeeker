...
Remember: You are a systematic trading model. Make principled decisions based on quantitative data and advanced technical analysis.""",
            ...
            response = requests.post(
                self.base_url, json=data, headers=headers, timeout=120
...
class RealMarketData:
    ...
    def _validate_kline_data(self, df: pd.DataFrame, symbol: str, interval: str) -> bool:
        ...
        if volume_mean < 1000:  # Minimum average volume threshold
            print(
                f"⚠️ Insufficient volume for {symbol} ({interval}): avg volume {volume_mean:.0f} < 1000"
            )
            return False
    ...
    def get_technical_indicators(self, coin: str, interval: str) -> dict[str, Any]:
        ...
        try:
            ema_20_series = self.calculate_ema_series(close_prices, 20)
            ema_50_series = self.calculate_ema_series(close_prices, 50)
            rsi_14_series = self.calculate_rsi_series(close_prices, 14)
            ...
            avg_vol = df["volume"].rolling(window=20, min_periods=1).mean().iloc[-1]
                ...
                if coin == "SHIB":
                    ...
                    try:
                        response = requests.get(
                            f"{self.spot_url}/ticker/price?symbol=SHIBUSDT", timeout=5
                        )
                        response.raise_for_status()
                        data = response.json()
                        ...
                        print(
                            f"⚠️ SHIB price error: {e}. SHIB may not be available on Binance Futures."
                        )
                    ...
                    response = requests.get(
                        f"{self.spot_url}/ticker/price?symbol={coin}USDT", timeout=5
                    )
                    response.raise_for_status()
                    data = response.json()
                    price_val = float(data["price"])
...
class PortfolioManager:
    ...
    def __init__(self, initial_balance: float = None):
        ...
        self.override_file = "manual_override.json"
    ...
    def add_to_cycle_history(self, cycle_number: int, prompt: str, thoughts: str, decisions: dict):
        cycle_data = {
            ...
            "user_prompt_summary": prompt[:300] + "..." if len(prompt) > 300 else prompt,
    ...
    def calculate_risk_usd(
        self, entry_price: float, stop_loss: float, quantity: float, direction: str
    ) -> float:
    ...
    def check_and_execute_tp_sl(self, current_prices: dict[str, float]):
        ...
        for coin, position in list(self.positions.items()):  # Iterate over a copy for safe deletion
            ...
            if exit_decision["action"] == "close_position":
                ...
                print(
                    f"📈 TRAILING STOP UPDATE {coin}: New stop at ${format_num(exit_decision['new_stop'], 4)}"
                )
                continue
    ...
    def enhanced_exit_strategy(self, position: dict, current_price: float) -> dict[str, Any]:
        ...
        if current_margin <= max_limit:
            print(
                f"🛑 Position at maximum limit: ${current_margin:.2f} <= ${max_limit:.2f}. Closing position."
            )
            return {
                "action": "close_position",
                "reason": f"Position at maximum limit (${max_limit:.2f})",
            }
        ...
        if direction == "long":
            ...
            if unrealized_pnl_percent <= -0.01:  # 1.0% loss (sıkıştırılmış)
                print(
                    f"🛑 LOSS CUTTING: {direction} {position['symbol']} at {unrealized_pnl_percent * 100:.1f}% loss. Closing position."
                )
                return {
                    "action": "close_position",
                    "reason": f"Loss cutting at 1.0% loss ({unrealized_pnl_percent * 100:.1f}%)",
                }
        ...
        if direction == "long":
            ...
            if unrealized_pnl_percent >= level3:  # Level 3 profit - take 75%
                ...
                adjusted_percent = self._adjust_partial_sale_for_max_limit(
            ...
            if unrealized_pnl_percent >= level2:  # Level 2 profit - tighter trailing stop
                ...
                if current_price >= trailing_stop:
                    ...
                    return {
                        ...
                        "reason": f"Tight trailing stop at {level2 * 100:.1f}% profit",
                    }
                ...
                if current_price >= trailing_stop:
                    ...
                    return {
                        "action": "update_stop",
                        "new_stop": trailing_stop,
                        "reason": f"Normal trailing stop at {level1 * 100:.1f}% profit",
                    }
    ...
    def _calculate_maximum_limit(self) -> float:
        ...
        print(
            f"📊 Maximum limit: ${max_limit:.2f} (${15.0} fixed vs ${max_from_percentage:.2f} 15% of ${self.current_balance:.2f} available cash)"
        )
        return max_limit
    ...
    def _is_counter_trend_trade(
        self, coin: str, signal: str, indicators_3m: dict, indicators_4h: dict
    ) -> bool:
    ...
    def apply_market_regime_adjustment(
        self, confidence: float, signal: str, market_regime: str
    ) -> float:
        ...
        if market_regime == "BEARISH" and signal == "buy_to_enter":
            ...
            print(
                f"📊 Market regime adjustment: BEARISH market, LONG signal → confidence {confidence:.2f} → {adjusted_confidence:.2f}"
            )
            ...
            adjusted_confidence = confidence * 0.7
            print(
                f"📊 Market regime adjustment: BULLISH market, SHORT signal → confidence {confidence:.2f} → {adjusted_confidence:.2f}"
            )
            return adjusted_confidence
        else:
    ...
    def validate_counter_trade(
        ...
        try:
            ...
            if (signal == "buy_to_enter" and rsi_3m < 25) or (
                signal == "sell_to_enter" and rsi_3m > 75
            ):
                required_conditions += 1
            ...
            if (signal == "buy_to_enter" and macd_3m > macd_signal_3m) or (
                signal == "sell_to_enter" and macd_3m < macd_signal_3m
            ):
                required_conditions += 1
            ...
            return {
                ...
                "reason": f"Counter-trade validation: {required_conditions}/5 conditions met"
                if valid
                else f"Insufficient conditions: {required_conditions}/5",
            }
    ...
    def calculate_confidence_based_margin(self, confidence: float, available_cash: float) -> float:
        ...
        print(
            f"📊 Confidence-based margin: ${margin:.2f} (confidence: {confidence:.2f}, available cash: ${available_cash:.2f})"
        )
        return margin
    ...
    def execute_decision(self, decisions: dict[str, dict], current_prices: dict[str, float]):
        ...
        for coin, trade in decisions.items():
            ...
            if signal == "buy_to_enter" or signal == "sell_to_enter":
                ...
                confidence = min(
                    1.0, confidence + (volume_quality_score / 1000)
                )  # Add small confidence boost
                ...
                if stop_loss is not None:
                    ...
                    if signal == "buy_to_enter":
                        stop_loss = current_price - (
                            (current_price - stop_loss) * stop_loss_multiplier
                        )
                    else:  # sell_to_enter
                        stop_loss = current_price + (
                    ...
                    print(
                        f"📊 Dynamic Stop-Loss: {coin} için {stop_loss_multiplier}x multiplier uygulandı"
                    )
                ...
                try:
                    ...
                    is_counter_trend = self._is_counter_trend_trade(
                        coin, signal, indicators_3m, indicators_4h
                    )
                    ...
                    if is_counter_trend:
                        ...
                        validation_result = self.validate_counter_trade(
                            coin, signal, indicators_3m, indicators_4h
                        )
                        ...
                        if validation_result["valid"]:
                            ...
                            print(
                                f"   Conditions met: {validation_result.get('conditions_met', [])}"
                            )
                        else:
                ...
                calculated_margin = self.calculate_confidence_based_margin(
                    confidence, self.current_balance
                )
                ...
                if calculated_margin < 10.0:
                    print(
                        f"⚠️ Calculated margin ${calculated_margin:.2f} below minimum $10 coin amount. Trade blocked."
                    )
                    continue
                ...
                actual_risk_usd = self.calculate_risk_usd(
                    current_price, stop_loss, quantity_coin, direction
                )
...
class AlphaArenaDeepSeek:
    ...
    def enhanced_trend_detection(self, coin: str) -> dict[str, Any]:
        ...
        try:
            ...
            ema_comparison = (
                f"20-Period EMA: {format_num(ema20_4h)} vs. 50-Period EMA: {format_num(ema50_4h)}"
            )
            ...
            if direction == "long":
                ...
                if rsi_14 > 70:
                    invalidation_condition = (
                        "If 4H RSI breaks back below 60, signaling momentum failure"
                    )
                elif rsi_14 < 40:
                    invalidation_condition = (
                        "If 4H RSI breaks above 50, signaling momentum recovery"
                    )
                else:
                    invalidation_condition = (
                        "If 4H price closes below 4H EMA20, signaling trend reversal"
                    )
                ...
                if rsi_14 < 30:
                    invalidation_condition = (
                        "If 4H RSI breaks back above 40, signaling momentum failure"
                    )
                elif rsi_14 > 60:
                    invalidation_condition = (
                        "If 4H RSI breaks below 50, signaling momentum recovery"
                    )
                else:
                    invalidation_condition = (
                        "If 4H price closes above 4H EMA20, signaling trend reversal"
                    )
    ...
    def format_position_context(self, position_context: dict) -> str:
        ...
        for symbol, data in position_context.items():
            ...
            formatted += (
                f"  {symbol}: ${pnl:.2f} PnL, {progress}% to target, {time_in_trade}min in trade\n"
            )
        return formatted
...
Current live positions & performance:"""
    ...
    def parse_ai_response(self, response: str) -> dict:
        ...
        thoughts = "Error: Could not parse AI response."
        decisions = {}
        try:
            parts = response.split("DECISIONS", 1)
            if len(parts) == 2:
                thoughts_raw = parts[0].replace("CHAIN_OF_THOUGHTS", "").strip()
                json_part = parts[1].strip()
                thoughts = thoughts_raw if thoughts_raw else "No thoughts."
            else:
                print("⚠️ AI response keywords missing.")
                json_part = response
                thoughts = "Keywords missing."
    ...
    def adjust_position_size_by_trend(
        self, base_size: float, market_regime: str, signal: str
    ) -> float:
            ...
            if isinstance(decisions, dict) and decisions and not manual_override:
                ...
                if has_close_position_signal:
                    ...
                    for coin, trade in decisions.items():
                        ...
                        if (
                            ...
                            if coin in valid_prices:
                                ...
                                print(
                                    f"✅ AI CLOSE: Closed {direction} {coin} @ ${format_num(current_price, 4)} (PnL: ${format_num(profit, 2)})"
                                )
                    ...
                    self.portfolio._execute_new_positions_only(
                        decisions, valid_prices, cycle_number
                    )
                else:
                    # Normal karar işleme (partial profit aktif)
                    self.portfolio._execute_normal_decisions(
                        decisions, valid_prices, cycle_number, positions_closed_by_tp_sl
                    )
        ...
        while self.is_running:
            try:
                ...
                if valid_prices:
                    ...
                    if positions_closed:
                        print("⏰ 45-SECOND TP/SL CHECK: Positions closed")
                    else:
                        print("⏰ 45-SECOND TP/SL CHECK: No triggers")
                else:
    ...
    def run_simulation(self, total_duration_hours: int = 168, cycle_interval_minutes: int = 2):
        ...
        print("   TP/SL Monitoring: Enabled (1 minute interval)")
