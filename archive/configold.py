...
class Config:
    ...
    MAX_TRADE_NOTIONAL_USD: float = float(
        os.getenv("MAX_TRADE_NOTIONAL_USD", "200.0")
    )  # Nof1ai blog: $12k-$68k positions
    ...
    MARKET_REGIME_MULTIPLIERS: dict = {"BULLISH": 1.0, "BEARISH": 0.7, "NEUTRAL": 0.85}
    ...
    )  # Minimum $10 margin ile pozisyon açılabilir
    MIN_POSITION_NOTIONAL_USD: float = float(
        os.getenv("MIN_POSITION_NOTIONAL_USD", "10.0")
    )  # Minimum $10 pozisyon (backward compatibility)
    ...
    MIN_PARTIAL_PROFIT_REMAINING_USD: float = float(
        os.getenv("MIN_PARTIAL_PROFIT_REMAINING_USD", "10.0")
    )  # Minimum $10 kalacak şekilde satış (backward compatibility)
