...
1. `RealMarketData` veriyi Binance'den çeker ve cache'ler.
...
- **`portfolio_manager.py`**: Risk ve Durum Yönetimi. PnL hesaplama, state takibi ve $O(1)$ hızında `Dirty-Cache` yönetimi.
...
- **`data_engine.py`**: Veri Ambarı. SQLite işlemleri, eğitim verisi toplama (`decisions` tablosu) ve ML özellik hazırlığı.
...
1.  **Market Fetch**: `RealMarketData` 3m, 15m ve 1h verilerini tek bir parallel fetch ile çeker.
2.  **UDP Cache**: Çekilen veriler UDP üzerinden belleğe yazılır.
3.  **ML Consensus**: `MLService`, 15m verisini cache'den alır ve XGBoost üzerinden olasılıkları (Buy/Sell/Hold) üretir.
4.  **AI Analysis**: `AIService` piyasa verisi + ML tahmini ile kapsamlı bir prompt oluşturur ve DeepSeek-V3'e sorar.
5.  **Risk Check**: AI "GİR" derse, `PortfolioManager` kasa limitlerini kontrol eder.
6.  **Smart Entry**: `AccountService`, emir defterini analiz eder ve **Limit Order** girer.
7.  **Execution Logging**: İşlem SQLite ve JSONL dosyalarına feedback için kaydedilir.
