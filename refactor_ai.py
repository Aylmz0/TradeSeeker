...
def main():
    ...
    if "self.ai_service = AIService(" not in main_code:
        main_code = main_code.replace("        self.strategy_analyzer = StrategyAnalyzer(self.market_data)\n", init_stmt)

    # Replace self.generate_alpha_arena_prompt_json with self.ai_service.generate_alpha_arena_prompt_json
    main_code = main_code.replace("self.generate_alpha_arena_prompt_json", "self.ai_service.generate_alpha_arena_prompt_json")
    main_code = main_code.replace("self.generate_alpha_arena_prompt", "self.ai_service.generate_alpha_arena_prompt")
